...
func ToPublishVariableHeader(fixedHeader FixedHeader, bs []byte) (PublishVariableHeader, error) {
	...
	if len(bs) < 2 {
		return PublishVariableHeader{}, fmt.Errorf("len(bs) should be >= 2")
	}
	...
	if len(bs) < (2 + int(lengthLSB)) {
		return PublishVariableHeader{}, fmt.Errorf("len(bs) should be >= 2+LSB")
	}
